...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="每日增量维护领星产品自定义字段，只写新增或字段值发生变化的SKU")
    ...
    p.add_argument("--force", action="store_true", help="忽略历史成功日志，强制写入筛选范围内所有SKU")
...
def build_fields(row: dict[str, Any]) -> list[dict[str, str]]:
    out = []
    for name, val in expected_values(row).items():
        if val:
...
def normalize_fields(fields: Any) -> list[dict[str, str]]:
    ...
    return sorted(out, key=lambda x: (x["id"], x["name"], x["val"]))
...
def load_latest_success_fields(db: Database) -> dict[str, list[dict[str, str]]]:
    ...
    for r in db.fetch_all(sql):
        out[clean(r.get("sku"))] = normalize_fields(r.get("fields_json"))
    return out


def select_changed(rows: list[dict[str, Any]], latest: dict[str, list[dict[str, str]]], force: bool) -> tuple[list[dict[str, Any]], Counter]:
    counter: Counter[str] = Counter()
    selected: list[dict[str, Any]] = []
    for r in rows:
        current = normalize_fields(build_fields(r))
        prev = latest.get(clean(r.get("sku")))
        if force:
            r["_change_reason"] = "force"
            selected.append(r)
            counter["force"] += 1
        elif not prev:
            r["_change_reason"] = "new"
            selected.append(r)
            counter["new"] += 1
        elif prev != current:
            r["_change_reason"] = "changed"
            selected.append(r)
            counter["changed"] += 1
        else:
            counter["skipped_unchanged"] += 1
...
async def main() -> None:
    a = parse_args()
    batch_no = a.batch_no or "custom_fields_incremental_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    db = Database()
    db.execute(DDL)
    rows = load_rows(db, a)
    latest = {} if a.force else load_latest_success_fields(db)
    targets, stat = select_changed(rows, latest, a.force)
    ...
    for r in targets[: a.show]:
        print(r["sku"], r.get("_change_reason"), "->", build_fields(r), clean(r.get("task_text")))
    if not a.confirm:
        print("预览模式，未写入。")
        return
    ...
    for i, r in enumerate(targets, 1):
        ...
        if i % 100 == 0 or i == len(targets):
            print(f"进度：{i}/{len(targets)} {dict(counter)}")
        if a.delay:
